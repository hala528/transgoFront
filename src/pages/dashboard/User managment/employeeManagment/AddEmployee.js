...
async function handleSubmit(e) {
  ...
  try {
    ...
    setLoading(false);
    setSuccess(res.data.message || t("addEmployee.addSuccess"));
    setTimeout(() => {
    Navigate("/dashboard/employee"); // غيرها حسب الراوت عندك
    }, 2000);
    ...
    return(
        <div className="w-100 p-2">
             <h3 style={{color:'white', flex:1 ,padding:5}}>{t("addEmployee.pageTitle")}</h3>
              <h3 className="subtitle-driver-bold">{t("addEmployee.enterEmployeeInfo")}</h3>
      ...
      color: 'white',
       ...
       <p className="subtitle-driver">{t("addEmployee.enterFullName")}</p>
        <Col >
          <Form.Control 
          placeholder={t("addEmployee.enterFullName")}
          name="full_name"
          onChange={handleChange}
          className="custom-input-driver"
    style={{
      ...
      background:'rgba(255, 255, 255, 0.15)'
        ...
        <p className="subtitle-driver">{t("addEmployee.enterPhoneNumber")}</p>
        <Col >
          <Form.Control 
          placeholder={t("addEmployee.enterPhoneNumber")}
          name="phone"
          onChange={handleChange}
          className="custom-input-driver"
    style={{
      ...
      background:'rgba(255, 255, 255, 0.15)'
        ...
        <p className="subtitle-driver">{t("addDriver.enterEmail")}</p>
        <Col >
          <Form.Control 
    ...
    style={{
      ...
      background:'rgba(255, 255, 255, 0.15)'
        ...
        <p className="subtitle-driver">{t("addEmployee.enterRole")}</p>
        <Col >
         <Form.Select name="role" onChange={handleChange} className="custom-selectt"
    style={{
      borderRadius: '3px',
      ...
      <option>{t("addEmployee.openSelectRole")}</option>
      ...
      <option value="admin">{t("appUsageReport.admin")}</option>
        ...
        <p className="subtitle-driver">{t("addEmployee.enterPassword")}</p>
        <Col >
          <Form.Control 
          placeholder={t("addEmployee.enterPassword")}
          name="password"
          type="password"
          onChange={handleChange}
          className="custom-input-driver"
    style={{
      ...
      <button className="btn-login" onClick={handleSubmit} disabled={loading}>
  {loading ? t("common.loading") : t("addEmployee.addEmployeeBtn")}
</button>
{success && <span className="success">{success}</span>}
{err && <span className="error">{err}</span>}
        </div>
